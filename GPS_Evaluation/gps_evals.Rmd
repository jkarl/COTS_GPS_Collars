...
g1 <- ggplot(data=plot.df, aes(x=FixTime/1000))+geom_histogram(binwidth=5)+
  ...
  labs(title="Time to First Fix", x="Seconds", y="# of Fixes")
g1
...
g2 <- ggplot(data=plot.df, aes(x=BestTime/1000))+geom_histogram(binwidth=5)+
  ...
  labs(title="Time to Best Fix - 10 minute fixes, indoors", x="Seconds", y="# of Fixes")
g2
...
g <- ggplot(data=plot.df[plot.df$HDOP<500,], aes(x=diffLon,y=diffLat,color=HDOP))+geom_point()+
  ...
  labs(title="GPS positional error - 10 minute fixes, indoors", x="Difference from Avg Longitude (m)", y="Difference from Avg Latitude (m)")
g
```
...
```
```{r, include=FALSE}
mean(plot.df$FixTime)
quantile(plot.df$FixTime, 0.9)

mean(plot.df$BestTime)
quantile(plot.df$BestTime, 0.9)

mean(plot.df$diffdist)
quantile(plot.df$diffdist, 0.9)
```
